...
class RobertaEmbeddings(nn.Module):
    ...
    def __init__(self, model, config):
        ...
        self.entity_embeddings = nn.Embedding(
            9, config.hidden_size, padding_idx=0)
    ...
    def create_entity_ids_from_input_ids(self, input_ids):
        ...
        for i in range(len(s_ids)):
            ...
            o_id = o_ids[i]
            if i % 2 == 0:
                entity_ids[s_id[0], s_id[1] +
                           2] = type_map[input_ids[s_id[0], s_id[1] + 2].item()]
                entity_ids[o_id[0], o_id[1] +
                           2] = type_map[input_ids[o_id[0], o_id[1] + 2].item()]
            else:
                prev_s_id = s_ids[i - 1]
                prev_o_id = o_ids[i - 1]
